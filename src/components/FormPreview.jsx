...
const checkDependencies = (field, sampleData) => {
    // Check if dependsOn exists and has rules
    ...
    const checkRule = (rule) => {
        ...
        switch (rule.condition) {
            case 'isTrue':
                conditionMet = targetValue === true;
                break;
            case 'isFalse':
                conditionMet = targetValue === false;
                break;
            case 'notEmpty':
                conditionMet = targetValue !== '' && targetValue !== null && targetValue !== undefined;
                break;
            case 'isEmpty':
                conditionMet = targetValue === '' || targetValue === null || targetValue === undefined;
                break;
            case 'equals':
                conditionMet = String(targetValue) === String(rule.value);
                break;
            case 'notEquals':
                conditionMet = String(targetValue) !== String(rule.value);
                break;
            case 'contains':
                conditionMet = String(targetValue).includes(String(rule.value));
                break;
            case 'doesNotContain':
                conditionMet = !String(targetValue).includes(String(rule.value));
                break;
            case 'startsWith':
                conditionMet = String(targetValue).startsWith(String(rule.value));
                break;
            case 'endsWith':
                conditionMet = String(targetValue).endsWith(String(rule.value));
                break;
            case 'greaterThan':
            case 'lessThan':
    ...
    if (logic === 'OR') {
        return rules.some(rule => checkRule(rule));
    } else {
        return rules.every(rule => checkRule(rule));
    }
};
...
const FieldRenderer = React.memo(function FieldRenderer({
    ...
    const {
        onOpenInputBuilderForEdit, onMoveField, onDeleteField, onDuplicateField,
        onSelectField, selectedFieldId, sampleData, onSampleDataChange
    } = callbacks;

    if (!field) return null;

    const isEnabled = checkDependencies(field, sampleData);
    const isSelected = selectedFieldId === field.id;
    
    // Determine the action if dependencies are not met (default to 'disable')
    const action = field.dependsOn?.action || 'disable';

    const fieldWrapperSx = {
        position: 'relative', p: '1px', my: 1, borderRadius: 2, border: '2px solid',
        borderColor: isSelected ? 'primary.main' : 'transparent', cursor: 'pointer',
        transition: 'background-color 0.2s ease-in-out, border-color 0.2s ease-in-out, opacity 0.3s',
        // --- THIS IS THE KEY FIX ---
        // Only hide the field if the action is 'hide' and conditions are not met
        display: !isEnabled && action === 'hide' ? 'none' : 'block',
        // Still apply opacity for the 'disable' case to give a visual cue
        opacity: isEnabled ? 1 : 0.6,
        '&:hover': {
            backgroundColor: 'action.hover',
            '& > .field-controls': { opacity: 1 }
        },
    };
    ...
    const commonInputProps = {
        ...
        disabled: !isEnabled // The input is always disabled if conditions are not met
    };

    const fieldContent = () => {
        switch (field.type) {
            case 'input': case 'text': case 'email': case 'number': case 'tel':
                return <TextField {...commonInputProps} type={field.type} variant="outlined"
                    value={sampleData[field.id] ?? field.defaultValue ?? ''}
                    onChange={(e) => onSampleDataChange(field.id, e.target.value)} />;
            
            case 'textarea':
                return <TextField {...commonInputProps} multiline rows={3} variant="outlined"
                    value={sampleData[field.id] ?? field.defaultValue ?? ''}
                    onChange={(e) => onSampleDataChange(field.id, e.target.value)} />;

            case 'dropdown':
                return (
                    <FormControl fullWidth margin="normal" required={field.required} variant="outlined" onClick={(e) => e.stopPropagation()} disabled={!isEnabled}>
                        <InputLabel>{field.label || field.title}</InputLabel>
                        <Select value={sampleData[field.id] ?? field.defaultValue ?? ''} onChange={(e) => onSampleDataChange(field.id, e.target.value)} label={field.label || field.title}>
                            {Array.isArray(field.options) && field.options.map((opt, i) => <MenuItem key={i} value={opt.value}>{opt.label}</MenuItem>)}
                        </Select>
            ...
            case 'radio_group':
                 return (
                    <FormControl component="fieldset" margin="normal" required={field.required} sx={{ width: '100%', p: 2, border: '1px solid rgba(0, 0, 0, 0.23)', borderRadius: 1 }} onClick={(e) => e.stopPropagation()} disabled={!isEnabled}>
                        <Typography variant="body1" sx={{ fontWeight: 'medium' }}>{field.label || field.title}</Typography>
                        <RadioGroup value={sampleData[field.id] ?? field.defaultValue ?? ''} onChange={(e) => onSampleDataChange(field.id, e.target.value)}>
                            {Array.isArray(field.options) && field.options.map((opt, i) => <FormControlLabel key={i} value={opt.value} control={<Radio />} label={opt.label} />)}
                        </RadioGroup>
                        {field.help && <FormHelperText>{field.help}</FormHelperText>}
                    </FormControl>
                );
            ...
            case 'checkbox':
                return <FormControlLabel control={<Checkbox checked={sampleData[field.id] ?? field.defaultValue ?? false} onChange={(e) => onSampleDataChange(field.id, e.target.checked)} onClick={(e) => e.stopPropagation()} required={field.required} />}
                    label={field.label || field.title} sx={{ my: 1, p: 1, display: 'block' }} disabled={!isEnabled} />;
            
            case 'date_picker':
                return <TextField {...commonInputProps} type="date" variant="outlined" InputLabelProps={{ shrink: true }}
                    value={sampleData[field.id] ?? field.defaultValue ?? ''}
                    onChange={(e) => onSampleDataChange(field.id, e.target.value)} />;
            ...
            case 'attachment':
                return (
                    <Box sx={{ my: 2, p: 1 }}>
                        <Typography variant="body1" sx={{ fontWeight: 'medium', mb: 1 }}>{field.label || field.title}</Typography>
                        <Button variant="outlined" onClick={(e) => e.stopPropagation()} disabled={!isEnabled}>Upload File</Button>
                        {field.help && <FormHelperText>{field.help}</FormHelperText>}
                    </Box>
                );
                        ...
                        <Box sx={{ pl: 2, borderLeft: '2px solid #ddd' }}>
                            {Array.isArray(field.children)
                                ? field.children.map((child, index) => (
                                    <FieldRenderer key={child.id} field={child} indexPath={[...indexPath, index]} arrayLength={field.children.length} callbacks={callbacks} />
                                ))
                                : <Typography variant="caption" sx={{ fontStyle: 'italic' }}>Dynamic Children from: {String(field.children)}</Typography>
                            }
                        </Box>
                    </>
                );
