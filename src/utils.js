...
/**
 * Flattens the nested field structure into a single array for dropdowns.
 * @param {Array} fields - The array of field objects.
 * @returns {Array<{id: string, title: string}>} - A flat array of all fields.
 */
export function flattenFields(fields) {
  const flat = [];
  function recurse(fieldArray) {
    if (!Array.isArray(fieldArray)) return;
    for (const field of fieldArray) {
      flat.push({ id: field.id, title: field.title || field.label || field.id });
